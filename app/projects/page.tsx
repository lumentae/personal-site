...
async function getRepositories(): Promise<Repository[]> {
  ...
  responses.forEach((res) => {
    if (!res.ok) {
      throw new Error(
        `Failed to fetch repositories from ${res.url}: ${res.status} ${res.statusText}`,
      );
    }
  });
...
async function getPinned(): Promise<PinnedRepository[]> {
  const res = await fetch("https://pinned.berrysauce.dev/get/lumentae", {
    next: { revalidate: 3600 },
  });
  if (!res.ok) {
    throw new Error(
      `Failed to fetch pinned repositories: ${res.status} ${res.statusText}`,
    );
  }
...
export default async function Projects() {
  ...
  return (
    <Common>
      <Header
        heading="Projects"
        undertext="These are some of the projects I made!"
        showLinks
      ></Header>
      <Content>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-7">
          {filteredRepos.map((repo) => (
            <Project
              ...
              createdAt={
                repo.created_at ? new Date(repo.created_at) : undefined
              }
            />
          ))}
        </div>
